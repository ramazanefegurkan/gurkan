...
M001 tamamlandı (S01–S06 tüm slice'lar complete). Backend: ASP.NET Core 10 Web API + PostgreSQL 16 (Docker, port 5434) + JWT auth + grup bazlı RBAC. 15 controller (Auth, Groups, Users, Properties, PropertyNotes, Tenants, RentPayments, ShortTermRentals, RentIncreases, Expenses, Bills, Documents, Dashboard, Notifications, Reports), 58+ endpoint, 77 integration test (18 S01 + 14 S02 + 13 S03 + 8 S04 + 8 S05 + 16 S06). Frontend: React + Vite + TypeScript (gurkan-ui/), JWT auth context, login sayfası, responsive sidebar layout, mülk listesi/detay/form sayfaları, not CRUD, kiracı yönetimi (CRUD, ödeme tablosu, kira artışı, sözleşme sonlandırma), kısa dönem kiralama (CRUD, platform takibi), gider yönetimi (CRUD, kategori badge'leri, tekrarlayan gider), fatura yönetimi (CRUD, durum badge'leri, ödendi işaretleme), döküman yönetimi (multipart upload, kategori badge'leri, download, silme), Dashboard (multi-currency summary cards, per-property breakdown, Excel/PDF export), Bildirimler (severity-colored notification cards). PropertyLayout tab navigation pattern (Detaylar / Kiracılar / Kısa Dönem / Giderler / Faturalar / Dökümanlar). Default route is /dashboard. Multi-currency destekli, grup bazlı erişim kontrolü backend ve frontend'de çalışıyor. Aylık kira ödemeleri otomatik oluşuyor, gecikme query-time tespit ediliyor, kira artışı gelecek ödemelere yansıyor. Dosya yükleme/indirme/silme lokal filesystem üzerinde çalışıyor. Dashboard gerçek veriyi gösteriyor. In-app bildirimler (kira gecikme, fatura yaklaşma, sözleşme bitiş, kira artışı) query-time hesaplanıyor. Excel (ClosedXML) ve PDF (QuestPDF) rapor export çalışıyor.
...
- [x] M001: Gayrimenkul Portföy Yönetimi — Kira, gider, fatura takibi, döküman yönetimi, dashboard, raporlama
